...
export default function Packages() {
    return (
        <section className="py-24 bg-forest text-sand relative overflow-hidden" id="packages">
            {/* Background Decor */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {packagesData.map((pkg, i) => (
                        <motion.div
                            ...
                            className={`relative bg-sand text-charcoal rounded-3xl p-8 flex flex-col shadow-xl transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 ${pkg.popular ? 'border-2 border-terracotta transform md:-translate-y-4' : 'border border-forest/10 mt-0 md:mt-4'
                                }`}
                        >
                            {pkg.popular && (
                                <div className="absolute -top-5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-terracotta to-softgold text-sand px-4 py-1 rounded-full text-sm font-bold flex items-center gap-1 shadow-lg">
                            ...
                            <div className="mb-6">
                                ...
                                <h3 className="font-heading text-3xl font-bold mb-4">{pkg.name}</h3>
                                <div className="flex items-baseline gap-1 mb-4">
                                    <span className="text-4xl font-bold text-terracotta">{pkg.price}</span>
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                                className={`w-full py-4 rounded-xl font-bold transition-colors shadow-md flex justify-center items-center gap-2 ${pkg.popular
